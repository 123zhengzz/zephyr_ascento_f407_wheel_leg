...
调试原则：先只让一个电机动 0.3 到 1 秒，确认反馈、方向和停止都正常，再进入下一步。
...
如果命令不写 `[ms]`，默认持续 `1000 ms`。如果写得超过 `5000 ms`，代码也会限制到 `5000 ms`。初学者建议每次只用 `300` 到 `800 ms`。
...
- 轮子的实际反馈要通过 `motor wheel status` 看，重点看 `erpm`、`motor_rpm`、`speed`、`current`、`duty` 和 `age`。
...
| 目标 | 命令 |
| --- | --- |
| 最轻微试转 | `motor wheel current left 100 300` |
...
| 左轮小电流 | `motor wheel current left 100 300` |  |  |
| 右轮小电流 | `motor wheel current right 100 300` |  |  |
...
3. 后续平衡方向不对时，再改 `src/control.c` 的轮子输出符号。
